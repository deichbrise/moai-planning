package org.planning.persistence.model.constraint;

import org.planning.persistence.model.AbstractModel;
import org.planning.persistence.model.Lecture;
import org.planning.persistence.model.Room;

import java.util.Set;

/**
 * @author pascalstammer
 * @version 16.02.17.
 */
public class LectureRoomContraint extends AbstractBinaryConstraint<Lecture, Room> {
    public LectureRoomContraint( final Lecture model, final Set<Room> supportedRelations ) {
        super( model, supportedRelations );
    }
}
